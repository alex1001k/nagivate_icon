...
def _validate_inputs(fact, dim, dim_by_id, driver_links):
    """Явная проверка входных CSV перед расчётом -- чтобы битый экспорт (например, не
    ...
    if missing_in_dim:
        examples = '\n    '.join(repr(x) for x in missing_in_dim[:5])
        problems.append(
            f"В fact_pokazateli.csv встречается {len(missing_in_dim)} id_pokaz, которых нет "
            f"в dim_pokazateli.csv. Примеры:\n    {examples}\n"
            "  Частая причина -- в исходном CSV текстовое поле (название/категория/описание) "
            "содержит запятые и не взято в кавычки, из-за чего колонки съехали при чтении. "
            "Проверьте: (1) реальный разделитель файла -- выгрузки из русского Excel часто "
            "используют ';' вместо ','; (2) число полей в проблемных строках, например:\n"
            "    import csv\n"
    ...
    if 'direction' in dim.columns:
        ...
        if bad_dir:
            problems.append(f"В dim_pokazateli.csv есть direction не 'up'/'down': {bad_dir}")
    ...
    if len(driver_links):
        ...
        if unknown_ids:
            problems.append(f"В driver_links.csv есть id_pokaz/driver_id вне dim_pokazateli.csv: {unknown_ids[:5]}")

    if problems:
        raise SystemExit("Входные CSV не прошли проверку:\n\n- " + "\n\n- ".join(problems))


_validate_inputs(fact, dim, dim_by_id, driver_links)
