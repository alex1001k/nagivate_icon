...
PLAN_PCT_CAP = 300  # % -- выше этого plan_pct почти всегда значит "план ~0 по ошибке",
                    # а не реальное перевыполнение в тысячи процентов; такие значения глушим
...
for pid, g in fact.groupby('id_pokaz'):
    ...
    for i, row in g.iterrows():
        ...
        if pd.notna(row['plan']) and row['plan'] != 0:
            _plan_pct = (cur / row['plan'] - 1) * 100
            # план на порядки меньше факта -- почти всегда битые/технические данные
            # (напр. 0.0001 вместо настоящего плана), а не отклонение в тысячи процентов;
            # такое не считаем валидным сигналом, иначе оно забивает собой top_deviation
            # и лезет в бриф как выполнение плана "1077229%"
            if abs(_plan_pct) <= PLAN_PCT_CAP:
                plan_pct = _plan_pct
...
for r in records:
    ...
    if r['has_deviation']:
        ...
        # 1. факт/прогноз + выполнение плана в скобках -- одной строкой. Используем уже
        # посчитанный r['plan_pct'] (не пересчитываем заново из fact/plan) -- он же null,
        # когда план=0 или неправдоподобно мал относительно факта (см. PLAN_PCT_CAP выше),
        # так что одна и та же защита работает и тут, и в заголовке, и в severity
        fact_line = f"{value_label}: {vu(r['fact'], unit)}"
        if r['plan_pct'] is not None:
            completion = 100 + r['plan_pct']
            fact_line += f" (Выполнение плана: {tag(f'{completion:.0f}%', good_for('plan', r, direction))})"
        elif r['plan'] is not None:
            # план формально есть, но % выполнения не показываем -- либо план=0, либо
            # цифра похожа на битые данные (см. комментарий выше)
            fact_line += f" (план: {vu(r['plan'], unit)})"
